...
class TestSetup:
    def __init__(self):
        ...
        self.fileslist = [[f,0.,0.,0.] for f in TEST_PATHS['parent'].glob('*.npy')]
...
def test_read_data_files(setup):
    ...
    photFileArray, fileList = read_data_files(TEST_PATHS['parent'], setup.fileslist)
...
def test_remove_targets_calibrated(setup):
    ...
    photFileArray, fileList = read_data_files(parentPath, setup.fileslist)
    assert photFileArray.shape[0] == 4
    photometry = remove_stars_targets(photFileArray, acceptDistance=5.0, targetFile=setup.targets, removeTargets=1)
    # 3 stars are removed because they are variable
    assert photometry.shape == (55,2)

# def test_calibrate_photometry(setup):
#     calibrate_photometry(targets=setup.targets, filterCode='B', starvar, closerejectd, variabilityMultiplier
#
